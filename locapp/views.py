...
def DestinationView(request):
    ...
    if request.user.is_authenticated:
        ...
        mapobj = folium.Map(width=1300, height=550, location=touristLocation, zoom_start=15)
        ...
        if request.method == "POST":
            ...
            if fm.is_valid():
                ...
                mapobj = folium.Map(width=1300, height=550, location=get_center_coordinates(tourist_lat, tourist_lon,dest_lat,dest_long),
                ...
                dest_details = DestinationDetails.objects.filter(destination_name = destination_)
        ...
        mapobj = mapobj._repr_html_()
        fm = DestinationForm()
        return render(request,'locapp/index.html', context={'form':fm, 'map': mapobj,'username':uname})
    else:
        return render(request, 'locapp/login.html')
